#!/usr/bin/env python3
"""
Create mobile-responsive dashboard with face illustration for makeup products
"""
...
# Top nationalities
top_nationalities = df['What is your nationality?'].value_counts().head(3).to_dict()
...
<head>
    ...
    <style>
        ...
        .forehead-area {{ top: 20px; left: 50%; transform: translateX(-50%); width: 120px; height: 40px; border-radius: 60px 60px 30px 30px; }}
        .eye-area {{ top: 80px; width: 80px; height: 30px; }}
        .eye-left {{ left: 50px; }}
        .eye-right {{ right: 50px; }}
        .cheek-area {{ top: 140px; width: 60px; height: 60px; }}
        .cheek-left {{ left: 40px; }}
        .cheek-right {{ right: 40px; }}
        .nose-area {{ top: 120px; left: 50%; transform: translateX(-50%); width: 40px; height: 60px; border-radius: 20px; }}
        .lip-area {{ bottom: 80px; left: 50%; transform: translateX(-50%); width: 80px; height: 35px; border-radius: 40px; background: rgba(255, 99, 132, 0.3); }}
        .chin-area {{ bottom: 30px; left: 50%; transform: translateX(-50%); width: 60px; height: 40px; border-radius: 30px; }}
...
<body>
    <div class="container-fluid">
        ...
        <div class="stats-container">
            ...
            <div class="stat-card">
                <div class="stat-number">{len(top_nationalities)}</div>
        ...
        <div class="mobile-stats">
            ...
            <div class="mobile-stat-row">
                <span class="mobile-stat-label">Cushion Foundation Users</span>
                <span class="mobile-stat-value">{stats['cushion_users']}</span>
            </div>
            <div class="mobile-stat-row">
                <span class="mobile-stat-label">Daily Sunscreen Users</span>
                <span class="mobile-stat-value">{stats['sunscreen_daily']}</span>
            </div>
        </div>
    ...
    <script>
        ...
        function showDetails(aid) {{
            ...
            // Create face visualization with product mapping
            const productAreas = {{
                'Primer/Makeup base': 'forehead-area',
                'Liquid foundation': 'forehead-area',
                'BB/CC cream': 'forehead-area',
                'Cushion foundation': 'cheek-area',
                'Concealer': 'eye-area',
                'Tinted moisturizer': 'nose-area',
                'Blush': 'cheek-area',
                'Highlighter': 'cheek-area',
                'Contour': 'cheek-area'
            }};

            let faceVisualization = `
                <div class="face-makeup-visual">
                    <div class="makeup-area forehead-area" title="Foundation/Base">
                        ${{userProducts.some(p => p.includes('foundation') || p.includes('Primer') || p.includes('BB') || p.includes('CC')) ? '✓' : ''}}
                        Base
                    </div>
                    <div class="makeup-area eye-area eye-left" title="Eye area">
                        ${{userProducts.some(p => p.toLowerCase().includes('concealer') || p.toLowerCase().includes('eye')) ? '✓' : ''}}
                        Eye
                    </div>
                    <div class="makeup-area eye-area eye-right" title="Eye area">
                        ${{userProducts.some(p => p.toLowerCase().includes('concealer') || p.toLowerCase().includes('eye')) ? '✓' : ''}}
                        Eye
                    </div>
                    <div class="makeup-area cheek-area cheek-left" title="Cheek area">
                        ${{userProducts.some(p => p.toLowerCase().includes('blush') || p.toLowerCase().includes('cushion')) ? '✓' : ''}}
                        Cheek
                    </div>
                    <div class="makeup-area cheek-area cheek-right" title="Cheek area">
                        ${{userProducts.some(p => p.toLowerCase().includes('blush') || p.toLowerCase().includes('cushion')) ? '✓' : ''}}
                        Cheek
                    </div>
                    <div class="makeup-area nose-area" title="Nose area">
                        ${{userProducts.some(p => p.toLowerCase().includes('concealer') || p.includes('Tinted')) ? '✓' : ''}}
                        Nose
                    </div>
                    <div class="makeup-area lip-area" title="Lip area">
                        ${{userProducts.some(p => p.toLowerCase().includes('lip')) ? '✓' : ''}}
                        Lip
                    </div>
                    <div class="makeup-area chin-area" title="Chin area">
                        ${{userProducts.some(p => p.includes('foundation') || p.includes('Concealer')) ? '✓' : ''}}
                        Chin
                    </div>
                </div>
            `;
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="info-section">
                            ...
                            ${{userProducts.length > 0 ?
                                '<ul class="mb-0">' + userProducts.map(prod => `<li>${{prod}}</li>`).join('') + '</ul>' :
                                '<p class="mb-0">No products specified</p>'
                            }}
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="info-section text-center">
                            ...
                            ${{faceVisualization}}
                            <div class="product-usage-legend">
                                <small class="text-muted">✓ indicates areas where participant uses makeup products</small>
                            </div>
                        </div>
                    </div>
                </div>
...
print(f"- More meaningful statistics added")
print(f"- Mobile responsive design implemented")
print(f"- Face illustration for makeup visualization added")
print(f"- Face photo size increased")
